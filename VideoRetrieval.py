...
def convert_to_suggestion_input(top_frames, list_frames):
    ...
    for sim, video_name, index in top_frames:
        ...
        suggestions_input.append((sim, video_name, frame_idx, timestamp))
...
def create_suggestion(retrieved_frames = []):
    ...
    # Each frame is a tuple (similarity_score, video_name, frame_number, timestamp)
    # We now create a new list, each element is (video_name, frame_number, timestamp, similarity_score, query_number)
    ...
    while l < len(combined_frames):
        ...
        for i in range(l, r):
            ...
            max_end_timestamp = start_timestamp + 60 # Each suggestion is maximum 60 seconds long
        ...
        if best_suggestion != (-1, -1, -1):
            ...
            suggestions.append({'num_unique': best_suggestion[2], 'max_sim': max_sim, 'frames': combined_frames[best_suggestion[0]:best_suggestion[1] + 1]})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder_path', type=str, default='D:/AIC24/Data', help='Path to the folder containing the data')
    parser.add_argument('--num_frames', type=int, default=10, help='Number of frames to retrieve')
    args = parser.parse_args()

    queries = args.query.split(',')
    suggestions_inputs = []
    for query in queries:
        top_frames, list_frames = retrieve_frames(query, args.folder_path, args.num_frames)
